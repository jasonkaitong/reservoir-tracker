...
export default function App() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try { const r = await window.storage.get("lr_visits_v2");   setVisits(JSON.parse(r.value)); } catch { setVisits(SEED); }
      try { const s = await window.storage.get("lr_settings_v1"); const p = JSON.parse(s.value); setSettings(p); setSettingsForm(p); } catch {}
      setReady(true);
    })();
  }, []);

  const saveVisits   = (v) => { try { window.storage.set("lr_visits_v2",   JSON.stringify(v)); } catch {} };
  const saveSettings = (s) => { try { window.storage.set("lr_settings_v1", JSON.stringify(s)); } catch {} };
